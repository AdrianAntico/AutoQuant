...
#### **AutoBanditSarima()**
#### **AutoBanditNNet()**
These functions are new and improved versions of the forecast:: models that run under the hood of AutoTS(). There is a Bayesian multi-armed bandit running the search for the best model configurations. You can select how you want the procedure to evalute the models along with many other useful features.

#### **AutoTBATS()**
This function is similar to the Bandit functions above except this one does not utilize the bandit algo. The inner working of TBATS utilizes forecast::auto.arima() to determine the AR and MA values to pass along to TBATS. Unfortunately, I would prefer if that function used a not stepwise version of arima() so I can test the models under a different evaluation criteria that I've shown to be extremely beneficial. 
...
#### **AutoH2oGLMCARMA()**
#### **AutoH2oMLCARMA()**
This is a unique function that utilizes machine learning models to generate time series forecasts. It replicates the time series models forecasting approach, where it predicts one step ahead and re-generates the model features, and makes the next forecast, etc. If you need to generate forecasts for say 100,000 unique series, you can  simply supply the grouping variables that identify which series is which, and generate forecasts for every one. This function is rich in feature engineering parameters.
...
data[, Adrian := data.table::fifelse(Adrian < 0.5, 0, 1)]

# Build Models
Results <- RemixAutoAI::AutoXGBoostClassifier(
  data,
  TrainOnFull = FALSE,
  ValidationData = NULL,
  TestData = NULL,
