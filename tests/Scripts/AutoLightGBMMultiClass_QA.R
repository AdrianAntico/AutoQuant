# Testing
LightGBM_QA_Results_MultiClass <- data.table::CJ(
  TOF = c(TRUE,FALSE),
...
# 2: FALSE    FALSE Failure                TRUE         2
# 3: FALSE     TRUE Failure               FALSE         3
# 4: FALSE     TRUE Failure                TRUE         4
# 5:  TRUE    FALSE Failure               FALSE         5

# AutoLightGBMMultiClass
# run = 1
# run = 4
# run = 5
for(run in seq_len(LightGBM_QA_Results_MultiClass[,.N])) {

  # Define values
  tof <- LightGBM_QA_Results_MultiClass[run, TOF]
  PartitionInFunction <- LightGBM_QA_Results_MultiClass[run, PartitionInFunction]
  gridtune <- LightGBM_QA_Results_MultiClass[run, GridTune]
  Tar <- "Adrian"
...
# TargetLevels = NULL # TestModel$TargetLevels
# ModelObject = NULL  # TestModel$Model
# ModelPath = getwd() # NULL
# ModelID = "Test_Model_1"
# ReturnFeatures = TRUE
# TransformNumeric = FALSE
# BackTransNumeric = FALSE
...
# Categorical encoding scoring ----
# data=ScoringData
# ML_Type=TargetType
# GroupVariables=CatFeatures
# TargetVariable=NULL
