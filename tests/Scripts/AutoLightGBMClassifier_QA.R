...
for(run in seq_len(LightGBM_QA_Results_Classifier[,.N])) {
  ...
  # Timer
  End <- Sys.time()
  LightGBM_QA_Results_Classifier[run, RunTimeScore := as.numeric(difftime(time1 = End, Start))]
  ...
  TestModel <- NULL
  Sys.sleep(5)
  RemixAutoML:::Post_Append_Helper(LightGBM_QA_Results_Classifier,'AutoLightGBMClassifier_QA')
}
