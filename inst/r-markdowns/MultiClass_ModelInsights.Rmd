    ...
    if(!is.null(TestData) && !is.null(AddPDPList_Line)) {
      for(g in AddPDPList_Line) {
        if(is.numeric(TestData[[g]])) {
          ...
          # Add Plots
          Test_ParDepPlots[[nam]][[g]] <- AutoQuant::ParDepCalPlots(
            data = TestData, 
    ...
    if(!is.null(AddPDPList_Line)) {
      for(g in AddPDPList_Line) {
        if(is.numeric(TrainData[[g]])) {
          ...
          # Add Plots
          Train_ParDepPlots[[nam]][[g]] <- AutoQuant::ParDepCalPlots(
            data = TrainData, 
            PredictionColName = 'p1', 
            TargetColName = paste0('Temp_', nam), 
            IndepVar = g, 
    ...
    if(!is.null(AddPDPList_Bar) && !is.null(TestData)) {
      for(g in AddPDPList_Bar) {
        if(!is.numeric(TestData[[g]])) {
          ...
          # Add Plots
          Test_ParDepCatPlots[[nam]][[g]] <- AutoQuant::ParDepCalPlots(
            data = TestData, 
            PredictionColName = 'p1', 
            TargetColName = paste0('Temp_', nam), 
            IndepVar = g, 
    ...
    if(!is.null(AddPDPList_Bar) && !is.null(TrainData)) {
      for(g in AddPDPList_Bar) {
        if(!is.numeric(TrainData[[g]])) {
          ...
          # Add Plots
          Train_ParDepCatPlots[[nam]][[g]] <- AutoQuant::ParDepCalPlots(
            data = TrainData, 
            PredictionColName = 'p1', 
            TargetColName = paste0('Temp_', nam), 
            IndepVar = g, 
...
if(is.null(RemixOutput)) {
  ...
  if(!is.null(TestData)) {
    ...
    for(tarlevel in TargetLevels) {
      ...
      Test_EvaluationPlot[[tarlevel]] <- suppressMessages(AutoQuant::EvalPlot(
  ...
  if(!is.null(TrainData)) {
    ...
    for(tarlevel in TargetLevels) {
      ...
      Train_EvaluationPlot[[tarlevel]] <- suppressMessages(AutoQuant::EvalPlot(
  ...
  if(!is.null(TestData)) {
    ...
    for(tarlevel in TargetLevels) {
      TestData[, p1 := get(tarlevel)]
      TestData[, paste0('Temp_', tarlevel) := data.table::fifelse(Predict == eval(tarlevel), 1, 0)]
      Test_ROCPlot[[tarlevel]] <- suppressMessages(AutoQuant::ROCPlot(
        data = TestData,
  ...
  if(!is.null(TrainData)) {
    ...
    for(tarlevel in TargetLevels) {
      TrainData[, p1 := get(tarlevel)]
      TrainData[, paste0('Temp_', tarlevel) := data.table::fifelse(Predict == eval(tarlevel), 1, 0)]
      Train_ROCPlot[[tarlevel]] <- suppressMessages(AutoQuant::ROCPlot(
        data = TrainData,
        TargetName = paste0('Temp_', tarlevel),
        SavePlot = FALSE,
        Name = ModelID,
  ...
  if(!is.null(TestData)) {
    ...
    for(tarlevel in TargetLevels) {
      TestData[, p1 := get(tarlevel)]
      TestData[, paste0('Temp_', tarlevel) := data.table::fifelse(Predict == eval(tarlevel), 1, 0)]
      Test_CumGainsChart[[tarlevel]] <- suppressMessages(AutoQuant::CumGainsChart(
        data = data.table::copy(TestData),
  ...
  if(!is.null(TrainData)) {
    ...
    for(tarlevel in TargetLevels) {
      TrainData[, p1 := get(tarlevel)]
      TrainData[, paste0('Temp_', tarlevel) := data.table::fifelse(Predict == eval(tarlevel), 1, 0)]
      Train_CumGainsChart[[tarlevel]] <- suppressMessages(AutoQuant::CumGainsChart(
        data = data.table::copy(TrainData),
        PredictedColumnName = 'p1',
        TargetColumnName = paste0('Temp_', tarlevel),
        NumBins = 20,
...
if(!is.null(Test_Importance)) {
  ...
  eval(AutoQuant:::VI_Plot(Type = 'catboost', VI_Data = Test_Importance, TopN = 15))
} else {
...
if(!is.null(Validation_Importance)) {
  ...
  eval(AutoQuant:::VI_Plot(Type = 'catboost', VI_Data = Validation_Importance, TopN = 15))
} else {
...
if(!is.null(Train_Importance)) {
  ...
  eval(AutoQuant:::VI_Plot(Type = 'catboost', VI_Data = Train_Importance, TopN = 15))
} else {
