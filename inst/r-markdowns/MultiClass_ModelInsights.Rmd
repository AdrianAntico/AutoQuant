...
if(!is.null(RemixOutput)) {
  ...
  Test_EvalMetrics <- list()
  for(nam in Test_EvalMetricss) Test_EvalMetrics[[nam]] <- RemixOutput[['PlotList']][[nam]]
  Train_EvaluationPlotss <- EvalPlotNames[which(EvalPlotNames %like% 'Train_EvaluationPlot_')]
  Train_EvaluationPlots <- list()
  for(nam in Train_EvaluationPlotss) Train_EvaluationPlots[[nam]] <- RemixOutput[['PlotList']][[nam]]
...
if(!is.null(RemixOutput)) {
  ...
  for(nam in Test_ParDepPlotss) {
    ...
    Test_ParDepPlots <- RemixOutput$PlotList[[paste0('Test_ParDepPlots_', nam)]]
  ...
  for(nam in Train_ParDepPlotss) {
    ...
    # Name to keep and remove
    Train_ParDepPlots <- RemixOutput$PlotList[[paste0('Train_ParDepPlots_', nam)]]
  ...
  for(nam in Test_ParDepPlotss) {
    ...
    # Name to keep and remove
    Test_ParDepCatPlots <- RemixOutput$PlotList[[paste0('Test_ParDepPlots_', nam)]]
  ...
  for(nam in Test_ParDepPlotss) {
    ...
    # Name to keep and remove
    Train_ParDepCatPlots <- RemixOutput$PlotList[[paste0('Train_ParDepPlots_', nam)]]
