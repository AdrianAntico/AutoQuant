...
if(is.null(RemixOutput)) {
  ...
  ### Test
  if(!is.null(TestData)) {
    Test_EvaluationBoxPlot <- AutoQuant::EvalPlot(
      data = data.table::copy(TestData), 
      PredictionColName = 'Predict', 
      TargetColName = TargetColumnName, 
      GraphType = 'boxplot', 
  ...
  ### Train
  if(!is.null(TrainData)) {
    Train_EvaluationBoxPlot <- AutoQuant::EvalPlot(
      data = data.table::copy(TrainData),
      PredictionColName = 'Predict',
      TargetColName = TargetColumnName,
      GraphType = "boxplot", 
...
if(!is.null(Test_EvaluationBoxPlot)) {
  eval(AutoQuant:::PlotlyConversion(Test_EvaluationBoxPlot))
} else {
...
if(!is.null(Train_EvaluationBoxPlot)) {
  eval(AutoQuant:::PlotlyConversion(Train_EvaluationBoxPlot))
} else {
...
## <font size="5">Residuals CopulaPlots</font>
<p>
...
print('Test Data Copula Plot')
if(!is.null(Test_CopulaPlot)) {
  eval(AutoQuant:::PlotlyConversion(Test_CopulaPlot))
} else {
  print('Test_CopulaPlot is NULL or TestData is NULL')
}
print('Train + Validation Data Copula Plot')
if(!is.null(Train_CopulaPlot)) {
  eval(AutoQuant:::PlotlyConversion(Train_CopulaPlot))
} else {
