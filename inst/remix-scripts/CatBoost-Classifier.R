...
if("Args Setup" == "Args Setup") {
  ...
  # Target Type
  TargetType_Default <- "classification"

  # Feature Tuning
  FeatureTune_Default <- TRUE
  LeaveOneOut_Default <- 4
  ...
  CostMatrixWeights_Default <- c(1L, 0L, 0L, 1L)
  EvalMetric_Default <- "MCC"
  grid_eval_metric_Default <- "MCC"
  LossFunction_Default <- "Logloss"
  ...
  OptionList <- list(
    ...
    optparse::make_option(opt_str = "--CostMatrixWeights", type = "numeric", default = CostMatrixWeights_Default, help = "TP profit, FP profit, FN profit, TN profit"),
    optparse::make_option(opt_str = "--EvalMetric", type = "character", default = EvalMetric_Default, help = "Evaluation metric for model"),
    ...
    optparse::make_option(opt_str = "--LossFunction", type = "character", default = LossFunction_Default, help = "Loss function for model"),
  ...
  ArgsList$Modeling$CatBoost[["CostMatrixWeights"]] <- CL_ArgsList_ML[["CostMatrixWeights"]]
  ArgsList$Modeling$CatBoost[["EvalMetric"]] <- CL_ArgsList_ML[["EvalMetric"]]
  ArgsList$Modeling$CatBoost[["grid_eval_metric"]] <- CL_ArgsList_ML[["grid_eval_metric"]]
  ArgsList$Modeling$CatBoost[["LossFunction"]] <- CL_ArgsList_ML[["LossFunction"]]
...
repeat {
  ...
  for(Run in Args$Loop) {
    ...
    Results <- RemixAutoML::AutoCatBoostClassifier(
      ...
      SaveInfoToPDF = if(FeatureTune || SavePDFOutput) FALSE else TRUE,
  ...
  Output <- RemixAutoML:::ML_TunePath(TargetType="classification", MetricName="Utility", Results.=Results, ArgsList=ArgsList, FeatureTune.=FeatureTune, GridTune.=GridTune, LeaveOneOut.=LeaveOneOut)
