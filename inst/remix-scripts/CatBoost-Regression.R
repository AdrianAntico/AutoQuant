...
if("Args Setup" == "Args Setup") {
  ...
  # Target Type
  TargetType_Default <- "regression"

  # Feature Tuning
  FeatureTune_Default <- FALSE
  LeaveOneOut <- 1
  ...
  TransformNumericColumns_Default <- NULL
  Methods_Default <- c("Asinh","Sqrt","Log","LogPlus1","Logit","Asin")

  # Loss function
  eval_metric_Default <- "RMSE"
  eval_metric_value_Default <- 1
  loss_function_Default <- "RMSE"
  loss_function_value_Default <- 1
  ...
  OptionList <- list(
    ...
    optparse::make_option(opt_str = "--TransformNumericColumns", type = "character", default = TransformNumericColumns_Default, help = "Columns to apply transformation"),
    optparse::make_option(opt_str = "--Methods", type = "character", default = Methods_Default, help = "Methods of transformation"),
    ...
    optparse::make_option(opt_str = "--eval_metric_value", type = "character", default = eval_metric_value_Default, help = "Eval metric value for training"),
    optparse::make_option(opt_str = "--loss_function", type = "character", default = loss_function_Default, help = "Loss function for training"),
    optparse::make_option(opt_str = "--loss_function_value", type = "character", default = loss_function_value_Default, help = "Loss function value for training"),
  ...
  ArgsList$Modeling$CatBoost[["TransformNumericColumns"]] <- CL_ArgsList_ML[["TransformNumericColumns"]]
  ArgsList$Modeling$CatBoost[["Methods"]] <- CL_ArgsList_ML[["Methods"]]
  ...
  ArgsList$Modeling$CatBoost[["eval_metric_value"]] <- CL_ArgsList_ML[["eval_metric_value"]]
  ArgsList$Modeling$CatBoost[["loss_function"]] <- CL_ArgsList_ML[["loss_function"]]
  ArgsList$Modeling$CatBoost[["loss_function_value"]] <- CL_ArgsList_ML[["loss_function_value"]]
...
repeat {
  ...
  for(Run in Args$Loop) {
    ...
    # Run CatBoost Model
    Results <- RemixAutoML::AutoCatBoostRegression(

      # Return services
      OutputSelection = if(FeatureTune) c("EvalMetrics") else ArgsList$Modeling$CatBoost[["ReturnServices"]],
      TransformNumericColumns = ArgsList$Modeling$CatBoost[["TransformNumericColumns"]],
      Methods = ArgsList$Modeling$CatBoost[["Methods"]],
      ...
      SaveInfoToPDF = if(FeatureTune) FALSE else if(SavePDFOutput) TRUE else FALSE,
      ...
      eval_metric_value = ArgsList$Modeling$CatBoost[["eval_metric_value"]],
      loss_function = ArgsList$Modeling$CatBoost[["loss_function"]],
      loss_function_value = ArgsList$Modeling$CatBoost[["loss_function_value"]],
    ...
    if(FeatureTune) {
      ...
      EvalMetrics <- Results$EvaluationMetrics$TestData
  ...
  Output <- RemixAutoML:::ML_TunePath(TargetType="regression", MetricName="RMSE", Results.=Results, ArgsList=ArgsList, FeatureTune.=FeatureTune, GridTune.=GridTune, LeaveOneOut.=LeaveOneOut)
