...
AutoH2OMLScoring <- function(ScoringData = NULL,
  ...
  # Initialize H2O----
  if(tolower(ModelType) == "standard") tryCatch({h2o::h2o.init(startH2O = FALSE, max_mem_size = MaxMem, nthreads = NThreads, enable_assertions = FALSE)}, error = function(x) h2o::h2o.init(startH2O = TRUE, max_mem_size = MaxMem, nthreads = NThreads, enable_assertions = FALSE))
