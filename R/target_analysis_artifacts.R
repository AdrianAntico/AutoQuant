...
generate_target_analysis_artifacts <- function(
  ...
  TargetVar <- if (!is.null(TargetVar) && length(TargetVar) > 0L) {
    as.character(TargetVar[1L])
  } else {
    NULL
  }

  TrendDateVar <- if (!is.null(TrendDateVar) && length(TrendDateVar) > 0L) {
    as.character(TrendDateVar[1L])
  } else {
    NULL
  }

  TrendGroupVar <- if (!is.null(TrendGroupVar)) {
