...
#' @param TreeMethod "hist" or "gpu_hist"
#' @param Trees = list("classifier" = 1000, "regression" = 1000)
#' @param eta = list("classifier" = 0.05, "regression" = 0.05)
#' @param max_depth = list("classifier" = 4L, "regression" = 4L)
#' @param min_child_weight = list("classifier" = 1.0, "regression" = 1.0)
#' @param subsample = list("classifier" = 0.55, "regression" = 0.55)
#' @param colsample_bytree = list("classifier" = 0.55, "regression" = 0.55)
#' @examples
#' \dontrun{
#'
...
AutoXGBoostHurdleCARMA <- function(data,
  ...
  # Run AutoXGBoostHurdleModel() and return list of ml objects ----
  if(DebugMode) print('Run AutoXGBoostHurdleModel() and return list of ml objects ----')
  TestModel <- AutoXGBoostHurdleModel(
  ...
  for(i in seq_len(FC_Periods+1L)) {
    ...
    if(i == 1L) {
      if(!is.null(GroupVariables)) {
        ...
        print("here yo")
        print(Step1SCore)
        Preds <- AutoXGBoostHurdleModelScoring(
