#' @title AutoXGBoostFunnelCARMA
#'
#' @description AutoXGBoostFunnelCARMA is a forecasting model for cohort funnel forecasting for grouped data or non-grouped data
#'
...
#'
#' # ML Args begin
#' @param NThreads = parallel::detectCores()
...
AutoXGBoostFunnelCARMA <- function(data,
  ...
  for(proc in Jobs) {
    ...
    if(proc %chin% c("evaluate","eval","training","train")) {
      ...
      # Build model ----
      x <- system.time(gcFirst = FALSE, TestModel <- AutoQuant::AutoXGBoostRegression(
...
AutoXGBoostFunnelCARMAScoring <- function(TrainData,
  ...
  while(TrainEndDate < ForecastEndDate) {
    ...
    temp <- AutoQuant::AutoXGBoostScoring(
