...
#' @param objective 'multiclass', 'multiclassova'
#' @param multi_error_top_k Default 1. Counts a prediction as correct if the chosen label is in the top K labels. K = 1 == multi_error
#' @param metric 'multi_logloss', 'multi_error', 'kullback_leibler', 'cross_entropy', 'cross_entropy_lambda'
...
#' @export
AutoLightGBMMultiClass <- function(data = NULL,
                                   TrainOnFull = FALSE,
                                   ValidationData = NULL,
                                   TestData = NULL,
  ...
  if(GridTune) {
    Output <- LightGBMGridTuner(ModelType="multiclass", TrainOnFull.=TrainOnFull, DebugMode.=DebugMode, params.=params, num_iterations.=params$num_iterations, max_depth.=params$max_depth, eta.=params$eta, num_leaves.=params$num_leaves, min_data_in_leaf.=params$min_data_in_leaf, bagging_freq.=params$bagging_freq, bagging_fraction.=params$bagging_fraction, feature_fraction.=params$feature_fraction, feature_fraction_bynode.=params$feature_fraction_bynode, lambda_l1.=params$lambda_l1, lambda_l2.=params$lambda_l2, LossFunction=NULL, EvalMetric=eval_metric, grid_eval_metric.=grid_eval_metric, CostMatrixWeights=CostMatrixWeights, TargetColumnName.=TargetColumnName, datatrain.=datatrain, dataTest.=dataTest, TestData.=TestData, EvalSets.=EvalSets, TestTarget.=TestTarget, FinalTestTarget.=FinalTestTarget, TargetLevels.=TargetLevels, MaxRunsWithoutNewWinner=MaxRunsWithoutNewWinner, MaxModelsInGrid=MaxModelsInGrid, MaxRunMinutes=MaxRunMinutes, BaselineComparison.=BaselineComparison, SaveModelObjects=SaveModelObjects, metadata_path=metadata_path, model_path=model_path, ModelID=ModelID, NumLevels.=NumLevels)
  ...
  if(!is.null(datatest)) {
    ...
    predict <- XGBoostMultiClassPredict(model=model, datatest=as.matrix(dataTrain), TargetLevels=TargetLevels, NumLevels=NumLevels, NumberRows=nrow(dataTrain))
  }
  ...
  if("score_traindata" %chin% tolower(OutputSelection) && !TrainOnFull) {
    predict <- XGBoostMultiClassPredict(model=model, datatest=as.matrix(dataTrain), TargetLevels=TargetLevels, NumLevels=NumLevels, NumberRows=nrow(dataTrain))
    if(!is.null(datatest)) {
      predict_validate <- XGBoostMultiClassPredict(model=model, datatest=as.matrix(dataTest), TargetLevels=TargetLevels, NumLevels=NumLevels, NumberRows=nrow(dataTest))
  ...
  MultinomialMetrics[["TestData"]] <- MultiClassMetrics(ModelClass="xgboost", DataType = "Test", SaveModelObjects.=SaveModelObjects, ValidationData.=ValidationData, PredictData.=predict, TrainOnFull.=TrainOnFull, TargetColumnName.=TargetColumnName, TargetLevels.=TargetLevels, ModelID.=ModelID, model_path.=model_path, metadata_path.=metadata_path, Debug = DebugMode)
  if("score_traindata" %chin% tolower(OutputSelection) && !TrainOnFull) {
    MultinomialMetrics[["TrainData"]] <- MultiClassMetrics(ModelClass="xgboost", DataType = "Train", SaveModelObjects.=SaveModelObjects, ValidationData.=ValidationData, PredictData.=predict, TrainOnFull.=TrainOnFull, TargetColumnName.=TargetColumnName, TargetLevels.=TargetLevels, ModelID.=ModelID, model_path.=model_path, metadata_path.=metadata_path, Debug = DebugMode)
  }
  ...
  if(ReturnModelObjects) {
    ...
    outputList[["TargetLevels"]] <- if(exists("TargetLevels") && !is.null(TargetLevels)) TargetLevels else NULL
