...
AutoLightGBMScoring <- function(TargetType = NULL,
  ...
  if(!is.null(EncodingMethod) && EncodingMethod == 'binary') {
    if(!is.null(FactorLevelsList)) {
      ScoringData <- Rodeo::DummifyDT(data=ScoringData, cols=names(FactorLevelsList), KeepFactorCols=FALSE, OneHot=FALSE, SaveFactorLevels=FALSE, SavePath=ModelPath, ImportFactorLevels=FALSE, FactorLevelsList=FactorLevelsList, ReturnFactorLevels=FALSE, ClustScore=FALSE, GroupVar=TRUE)
    } else {
  ...
  # ModelDataPrep Check ----
  ScoringData <- ScoringData[, .SD, .SDcols = FeatureColumnNames]
