...
#' @param NLag Number of rows back for the lag operation
...
AutoDiffLagN <- function(data,
  ...
  if(!is.numeric(NLag)) stop("NLag needs to be a numeric valued scalar")
  ...
  if(!is.null(GroupVariables)) {
    ...
    data <- data[, (ModDiffVariables) := data.table::shift(x = .SD, n = NLag, fill = NA, type = "lag"), .SDcols = c(DiffVariables)]
    data <- data[, (ModDiffVariables) := lapply(DiffVariables, DiffDT)]
  }
