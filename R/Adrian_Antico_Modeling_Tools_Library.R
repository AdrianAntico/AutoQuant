        ...
        if(cc < dd) {
          ...
          if(Construct[i,21][[1]] == TRUE) {
            ...
            if(tolower(Construct[i,22][[1]]) == "standard") {
              ...
              h2o.download_mojo(model = best_model, path = model_path, get_genmodel_jar = TRUE, genmodel_path = model_path)
              set(grid_tuned_paths, i = i, j = 2L, value = save_model)
              save(grid_tuned_paths, file = paste0(model_path, "/grid_tuned_paths.Rdata"))
            }
          }
          ...
          if(Construct[i,21][[1]] == TRUE) {
            ...
            if(tolower(Construct[i,22][[1]]) == "standard") {
              ...
              h2o.download_mojo(model = bl_model, path = model_path, get_genmodel_jar = TRUE, genmodel_path = model_path)
              set(grid_tuned_paths, i = i, j = 2L, value = save_model)
              save(grid_tuned_paths, file = paste0(model_path, "/grid_tuned_paths.Rdata"))
            }
          }
